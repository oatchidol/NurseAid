...
-- Durable patient-deletion jobs: coordinate PostgreSQL row deletion with the
-- (non-transactional, irreversible) InfluxDB interval deletes so a failure in
-- one store never silently loses telemetry while leaving PG rows behind. Jobs
-- created by DELETE /api/patients/:hn can be inspected and retried via the
-- /api/patient-deletions endpoints. interval list JSONB shape:
--   [{"mac":"..","start":iso,"stop":iso,"done":false}, ...]
CREATE TABLE IF NOT EXISTS patient_deletion_jobs (
    id SERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_patient_deletion_jobs_status ON patient_deletion_jobs(status);
CREATE INDEX IF NOT EXISTS idx_patient_deletion_jobs_hn ON patient_deletion_jobs(hn);

-- AI Assistant feedback: only request id, user id, and thumbs up/down are
-- kept -- never the prompt, the model's answer, or any vital-sign value.
