    ...
    while True:
        try:
            ...
            for session in result.get("sessions", [])[:4]:
                ...
                if response.exists():
                    payload = json.loads(response.read_text(encoding="utf-8")); content = redact_log_text(payload.get("content"))
                    if content: request_json("/api/v1/devices/log-batches", {"deviceId": credential["deviceId"], "sessionId": session_id, "content": content}, credential["credential"])
