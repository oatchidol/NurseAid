...
def process_log_requests(now=None):
    ...
    for request_path in list(SPOOL_DIR.glob("*.request.json"))[:8]:
        ...
        try:
            ...
            since = cursor_path.read_text().strip() if cursor_path.exists() else (now - timedelta(minutes=2)).isoformat()
            cursor = now.isoformat()
            ids = container_ids(service)
            if not ids: raise ValueError("container not found")
            output = command("docker", "logs", "--timestamps", "--since", since, ids[0])
            cursor_path.write_text(cursor, encoding="ascii")
            if output:
                atomic_response(response_path, {"sessionId": session_id, "service": service, "content": output[-48000:]})
...
def process_action_requests(now=None):
    ...
    for request_path in list(SPOOL_DIR.glob("*.action-request.json"))[:2]:
        ...
        try:
            ...
            elif service in allowed and service not in ("device-agent", "compose-collector"):
                ids = container_ids(service)
                if not ids: raise ValueError("container not found")
