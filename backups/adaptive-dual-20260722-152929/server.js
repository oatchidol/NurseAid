...
async function queryLiveStatuses() {
    ...
    const fluxQuery = `
        from(bucket: "${influxConfig.bucket}")
            ...
            |> filter(fn: (r) => r._measurement == "ble_heart" or r._measurement == "ble_spo2" or
                ...
                r._measurement == "ble_status" or r._measurement == "ble_batt" or
    ...
    return devicesResult.rows.map(device => {
        ...
        if (snapshot.connected && snapshot.worn) {
            if (snapshot.hrLive && hr !== '--' && (hr < Number(settings.hr_min) || hr > Number(settings.hr_max))) {
        ...
        const dataMessages = {
            offline: 'ไม่พบข้อมูลสดจากอุปกรณ์',
            off_wrist: 'อุปกรณ์ยืนยันว่าไม่ได้สวม · ซ่อนค่าทางคลินิกเก่าแล้ว',
            present_waiting: 'พบอุปกรณ์ในระยะ · ยังไม่มีค่าทางคลินิกสด',
            sensor_waiting: 'อุปกรณ์เชื่อมต่อ แต่ยังไม่ได้รับสัญญาณจากเซ็นเซอร์',
            recent: 'มีค่าล่าสุด แต่ HR ไม่อยู่ในช่วง real-time alarm'
        };
        const dataMessage = dataMessages[dataQuality]
            || (missingMetrics.length ? `ข้อมูลสดไม่ครบ: ${missingMetrics.join(', ')}` : 'HR สด · ค่ารอบอื่นเป็นปัจจุบัน');
        return {
    ...
    async function updateDash() {
        ...
        try {
            ...
            const cards = data.map(p => {
                ...
                const ages = p.metricAges || {};
                const hrAge = formatAge(ages.hr);
                const spo2Age = formatAge(ages.spo2);
                const tempAge = formatAge(ages.temp);
                const presenceAge = formatAge(ages.presence);

                const statusColor = isUnavailable
                    ? 'bg-red-400 shadow-[0_0_8px_rgba(248,113,113,0.45)]'
                    : isOffWrist
                    ? 'bg-slate-400'
                    : (isRecent || isPartial || isSensorWaiting || isPresentWaiting)
                ...
                const dataQualityLabels = {
                    live: 'HR สด · ใช้ alarm ได้',
                    recent: 'มีค่าล่าสุด · HR ไม่สด',
                    partial: 'ข้อมูลสดไม่ครบ',
                    present_waiting: 'เห็นอุปกรณ์ · รอข้อมูลคลินิก',
                ...
                const statusLabel = isUnavailable
                    ? 'ระบบข้อมูลขัดข้อง'
                    : (dataQualityLabels[dq] || (!isOnline ? 'ออฟไลน์' : 'พร้อมใช้งาน'));
                const statusLabelColor = isUnavailable
                    ? 'color: var(--accent-red);'
                    : (!isOnline || isOffWrist)
                    ? 'color: var(--text-tertiary);'
                    : (isLive ? 'color: var(--accent-green);'
                    : ((isRecent || isPartial || isSensorWaiting || isPresentWaiting) ? 'color: #f59e0b;' : 'color: var(--accent-green);'));
                const presenceLabel = presenceAge ? ('อุปกรณ์ ' + presenceAge) : '';
                const statusText = statusLabel + (presenceLabel ? ' · ' + presenceLabel : '');
                ...
                <div class="card p-4 border-t-4 transition-all" style="\${(isCrit || isUnavailable) ? 'border-color: var(--accent-red);' : ((isPartial || isSensorWaiting) ? 'border-color: #f59e0b;' : (isOnline ? 'border-color: var(--accent-green);' : 'border-color: var(--border-color);'))}">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <div class="p-2 rounded-xl text-center transition-all" \${hrBg}>
                            ...
                            <p class="text-[7px] font-bold mt-0.5" style="color: \${p.hrLive ? 'var(--accent-green)' : 'var(--text-tertiary)'};">\${p.hr !== '--' ? (p.hrLive ? '● สด' : (hrAge || '')) : ''}</p>
                        </div>
                        <div class="p-2 rounded-xl text-center transition-all" \${spo2Bg}>
                            ...
                            <p class="text-[7px] font-bold mt-0.5" style="color: var(--text-tertiary);">\${p.spo2 !== '--' && spo2Age ? spo2Age : ''}</p>
                        </div>
                        <div class="p-2 rounded-xl text-center transition-all" \${tempBg}>
                            ...
                            <p class="text-[7px] font-bold mt-0.5" style="color: var(--text-tertiary);">\${p.temp !== '--' && tempAge ? tempAge : ''}</p>
                        </div>
                    </div>
                </div>\`;
