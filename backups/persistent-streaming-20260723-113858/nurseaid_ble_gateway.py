...
JSTYLE_OFF_WRIST_CONFIRMATIONS = max(1, _env_int("JSTYLE_OFF_WRIST_CONFIRMATIONS", 2))
...
class NurseAidBLEGateway:
    ...
    async def _initialize_adapters(self) -> None:
        ...
        for runtime in self.adapter_manager.adapters.values():
            ...
            self.last_advertisement_by_adapter.setdefault(runtime.address, time.time())
        if not self.adapter_manager.adapters:
            print("[BLE] No configured adapter resolved; using BlueZ default compatibility mode")
            return
        mode = "dual-adapter adaptive" if self.dual_adapter_enabled else "single-adapter"
    ...
    def _operational_status(self) -> dict:
        ...
        active_measurements = sum(
            ...
            if state.get("connected") and state.get("monitor_phase") == "spo2"
        )
        adapters = {
            runtime.interface: {
        ...
        return {
            ...
            "adapterMode": "dual" if self.dual_adapter_enabled else "single",
            "adapters": adapters,
        }
    ...
    def _write_health_state(self, discovering: Optional[bool] = None):
        ...
        for address, runtime in self.adapter_manager.adapters.items():
            ...
            healthy = runtime.healthy and (
                active > 0 or adapter_measuring or age <= BLE_SCANNER_STALE_SECONDS
            )
            adapter_health[address] = {
                ...
                "lastAdvertisementAgeSeconds": round(age, 1),
            }
        if adapter_health:
        ...
        payload = {
            ...
            "mode": "dual" if self.dual_adapter_enabled else "single",
            "status": (
                "healthy-dual"
    ...
    def _record_device_data_success(self, mac: str) -> None:
        ...
        if address in self.adapter_manager.adapters:
            self.adapter_manager.record_data_result(mac, address, True)
    ...
    def _prioritize_connection_targets(self, targets):
        """Prioritize driver classes, then by SLA staleness (staler first), then
        fairly rotate by oldest connection attempt."""
        now = time.time()
    ...
    async def _start_adapter_scanner(self, adapter_address: str) -> bool:
        ...
        async with lock:
            ...
            try:
                ...
                self.scanner_started_at_by_adapter[address] = time.time()
    ...
    async def _stop_adapter_scanner_locked(self, adapter_address: str, force_bluez: bool):
        ...
        discovering = await self._adapter_discovering(address)
        if runtime and (force_bluez or stop_failed or discovering is True):
        ...
        try:
            ...
            while client.is_connected and not self.shutdown_event.is_set():
                ...
                if phase_result == "no_samples":
                    print(
                        f"[BLE] {mac}: no HR/Temp samples; ending cycle before SpO2"
                    )
                    break
            ...
            try:
                ...
                if address in self.adapter_manager.adapters:
                    ...
                    self.adapter_manager.record_data_result(
                        mac, address, bool(state.get("cycle_data_received"))
                    )
    ...
    async def _phase2_spo2(self, client: BleakClient, mac: str) -> bool:
        ...
        try:
            while client.is_connected and not self.shutdown_event.is_set():
                ...
                now = time.time()
                progress_reference = float(state.get("spo2_last_progress_at") or stream_started_at)
                remaining = min(hard_deadline - now, SPO2_NO_PROGRESS_TIMEOUT - (now - progress_reference))
                if remaining <= 0:
                    state["last_failure_reason"] = (
                        "spo2_timeout" if now >= hard_deadline else "spo2_no_progress"
                    )
                    break
                waiters = [
    ...
    async def _startup_adapter_cleanup(self):
        ...
        try:
            ...
            await asyncio.sleep(1.0)
            interfaces = [item.interface for item in self.adapter_manager.adapters.values()] or ["hci0"]
            for interface in interfaces:
                await self._run_shell(
    ...
    async def _connection_watchdog(self):
        ...
        while not self.shutdown_event.is_set():
            ...
            if self.dual_adapter_enabled:
                for address, runtime in self.adapter_manager.adapters.items():
                    last_adv = self.last_advertisement_by_adapter.get(address, now)
                    stale = (
                        runtime.active_connections == 0
                        and address in self.scanners
                        and now - last_adv > BLE_SCANNER_STALE_SECONDS
                    )
                    if not stale:
                        continue
                    print(
