...
### Adaptive Dual Bluetooth (production profile)

BLE Gateway รองรับ Bluetooth controller สองตัวแบบ adaptive โดยระบุ hardware
ด้วย public MAC แทนชื่อ `hci0`/`hci1` ซึ่งอาจสลับหลัง reboot:

```env
BLE_DUAL_ADAPTER_MODE=adaptive
BLE_ADAPTER_ADDRESSES=18:69:45:F3:45:77,2C:CF:67:54:42:05
BLE_MAX_GATT_CONNECTIONS=8
BLE_MAX_GATT_CONNECTIONS_PER_ADAPTER=4
BLE_MAX_ACTIVE_MEASUREMENTS_PER_ADAPTER=4
...
ด้วย controller ที่เหลือ นาฬิกา JStyle ที่ยืนยันการสวมใส่จะคง GATT notification
session และวัด HR/Temp/SpO2 แยกกันต่อเรือน ไม่มี global lock ระหว่างรอผลลัพธ์
