...
const parsedLiveFreshness = Number.parseInt(process.env.LIVE_VITAL_FRESHNESS_SECONDS || '180', 10);
const LIVE_VITAL_FRESHNESS_SECONDS = Number.isFinite(parsedLiveFreshness) && parsedLiveFreshness > 0
    ? parsedLiveFreshness
    : 180;
const LIVE_DATA_QUERY_WINDOW_MINUTES = Math.max(5, Math.ceil(LIVE_VITAL_FRESHNESS_SECONDS / 60) + 1);
...
async function queryLiveStatuses() {
    ...
    return devicesResult.rows.map(device => {
        ...
        const snapshot = buildLiveSnapshot(sensor, nowMs, LIVE_VITAL_FRESHNESS_SECONDS);
