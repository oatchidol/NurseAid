...
class CollectorTests(unittest.TestCase):
    ...
    @patch.object(collector, "log_command", return_value="nurseaid | safe log\n")
    @patch.object(collector, "command")
    def test_live_log_request_only_uses_allowlisted_compose_service(self, command, log_command, _chown):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(log_command.call_args.args, ("docker", "logs", "--timestamps", "--since", log_command.call_args.args[-2], "abc123"))

    @patch.object(collector.os, "chown")
    @patch.object(collector, "log_command", return_value="historical log\n")
    @patch.object(collector, "command")
    def test_first_live_log_request_uses_configured_history_window(self, command, log_command, _chown):
        session = "123e4567-e89b-42d3-a456-426614174000"
        now = datetime(2026, 7, 22, 7, 0, tzinfo=timezone.utc)
        command.side_effect = ["ble-gateway\n", "abc123\n"]
        with tempfile.TemporaryDirectory() as directory:
            spool = Path(directory)
            (spool / f"{session}.request.json").write_text(json.dumps({
                "service": "ble-gateway",
                "expiresAt": (now + timedelta(minutes=5)).isoformat(),
            }))
            with patch.object(collector, "SPOOL_DIR", spool), patch.object(
                collector, "INITIAL_LOG_HISTORY_MINUTES", 30
            ):
                collector.process_log_requests(now)
        self.assertEqual(log_command.call_args.args[-2], "2026-07-22T06:30:00+00:00")

    @patch.object(collector.os, "chown")
    @patch.object(collector, "log_command", return_value="x" * 60000)
    @patch.object(collector, "command")
    def test_live_log_response_is_limited_to_48kb(self, command, _logs, _chown):
        session = "123e4567-e89b-42d3-a456-426614174000"
        command.side_effect = ["nurseaid\n", "abc123\n"]
        with tempfile.TemporaryDirectory() as directory:
            spool = Path(directory)
            ...
            with patch.object(collector, "SPOOL_DIR", spool):
                collector.process_log_requests()
            response = json.loads((spool / f"{session}.response.json").read_text())
        self.assertEqual(len(response["content"]), 48000)

    @patch.object(collector.os, "chown")
    @patch.object(collector, "log_command", return_value="")
    @patch.object(collector, "command")
    def test_live_log_request_reports_idle_when_no_new_output(self, command, _logs, _chown):
        session = "123e4567-e89b-42d3-a456-426614174000"; command.side_effect = ["nurseaid\n", "abc123\n"]
        with tempfile.TemporaryDirectory() as directory:
            spool = Path(directory); (spool / f"{session}.request.json").write_text(json.dumps({"service": "nurseaid", "expiresAt": (datetime.now(timezone.utc)+timedelta(minutes=5)).isoformat()}))
            with patch.object(collector, "SPOOL_DIR", spool): collector.process_log_requests()
            self.assertEqual(json.loads((spool / f"{session}.response.json").read_text())["state"], "idle")

    @patch.object(collector.os, "chown")
    @patch.object(collector, "log_command", side_effect=__import__("subprocess").CalledProcessError(1, "docker logs", stderr="read failed"))
    @patch.object(collector, "command")
    def test_live_log_request_reports_docker_error(self, command, _logs, _chown):
        session = "123e4567-e89b-42d3-a456-426614174000"; command.side_effect = ["nurseaid\n", "abc123\n"]
        with tempfile.TemporaryDirectory() as directory:
            spool = Path(directory); (spool / f"{session}.request.json").write_text(json.dumps({"service": "nurseaid", "expiresAt": (datetime.now(timezone.utc)+timedelta(minutes=5)).isoformat()}))
            with patch.object(collector, "SPOOL_DIR", spool): collector.process_log_requests()
            self.assertEqual(json.loads((spool / f"{session}.response.json").read_text())["state"], "error")

    @patch.object(collector.subprocess, "run")
    def test_log_command_combines_stdout_and_stderr(self, run):
        run.return_value = __import__("types").SimpleNamespace(stdout="out\n", stderr="err\n")
        self.assertEqual(collector.log_command("docker", "logs", "abc"), "out\nerr\n")
    ...
    @patch.object(collector.os, "chown")
    @patch.object(collector, "command")
    def test_restart_action_rejects_non_compose_service(self, command, _chown):
        command.return_value = "nurseaid\ndevice-agent\ncompose-collector\n"
        for service in ("not-allowed",):
    ...
    @patch.object(collector, "wait_for_service", return_value={"status": "healthy", "healthStatus": "healthy"})
    @patch.object(collector, "command")
    def test_restart_action_allows_device_agent_and_waits_for_health(self, command, wait, _chown):
        action_id = "123e4567-e89b-42d3-a456-426614174000"
        command.side_effect = ["device-agent\ncompose-collector\n", "agent123\n", "agent123\n"]
        with tempfile.TemporaryDirectory() as directory:
            spool = Path(directory); request = spool / f"{action_id}.action-request.json"
            request.write_text(json.dumps({"action": "restart_service", "service": "device-agent", "expiresAt": (datetime.now(timezone.utc)+timedelta(minutes=5)).isoformat()}))
            with patch.object(collector, "SPOOL_DIR", spool): collector.process_action_requests()
            result = json.loads((spool / f"{action_id}.action-response.json").read_text())
            self.assertEqual(result["status"], "succeeded")
            self.assertIn(("docker", "restart", "agent123"), [call.args for call in command.call_args_list])
            wait.assert_called_once_with("device-agent")

    @patch.object(collector.os, "chown")
    @patch.object(collector, "restart_self")
    @patch.object(collector, "command")
    def test_collector_self_restart_writes_response_and_removes_request_first(self, command, restart_self, _chown):
        action_id = "123e4567-e89b-42d3-a456-426614174000"
        command.side_effect = ["device-agent\ncompose-collector\n", "collector123\n"]
        with tempfile.TemporaryDirectory() as directory:
            spool = Path(directory); request = spool / f"{action_id}.action-request.json"
            request.write_text(json.dumps({"action": "restart_service", "service": "compose-collector", "expiresAt": (datetime.now(timezone.utc)+timedelta(minutes=5)).isoformat()}))
            def verify_before_restart(container_id):
                self.assertEqual(container_id, "collector123")
                self.assertFalse(request.exists())
                self.assertTrue((spool / f"{action_id}.action-response.json").exists())
            restart_self.side_effect = verify_before_restart
            with patch.object(collector, "SPOOL_DIR", spool): collector.process_action_requests()
            result = json.loads((spool / f"{action_id}.action-response.json").read_text())
            self.assertEqual(result["status"], "succeeded")
            restart_self.assert_called_once_with("collector123")
