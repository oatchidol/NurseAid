...
if command -v bluetoothctl &>/dev/null; then
    ...
    BT_OUTPUT=$(bluetoothctl list 2>&1)
    CONTROLLER_COUNT=$(grep -c '^Controller ' <<<"$BT_OUTPUT" || true)

    if (( CONTROLLER_COUNT > 0 )); then
        pass_check "bluetoothctl พบ ${CONTROLLER_COUNT} controller(s)"
    else
    ...
    while read -r controller; do
        ...
        DETAIL=$(bluetoothctl show "$controller" 2>&1 || true)
        BT_DETAILS+=$'\n'"$DETAIL"
        if grep -q "Powered: yes" <<<"$DETAIL"; then
            pass_check "Controller ${controller}: Powered = yes"
        else
            fail_check "Controller ${controller}: Powered = no"
        fi
        if grep -q "Discoverable: yes" <<<"$DETAIL"; then
            fail_check "Controller ${controller}: Discoverable = yes"
        fi
    done < <(awk '/^Controller /{print $2}' <<<"$BT_OUTPUT")

    # Discovering
    if echo "$BT_DETAILS" | grep -q "Discovering: yes"; then
        pass_check "Bluetooth discovery กำลังทำงานบนอย่างน้อยหนึ่ง controller"
    else
        pass_check "Bluetooth Adapter: Discovering = no (ปกติ)"
    fi

    # Pairable
    if echo "$BT_DETAILS" | grep -q "Pairable: yes"; then
        fail_check "มี controller ที่ Pairable = yes (gateway ต้องเป็น no-pair mode)"
    else
    ...
    if echo "$BT_DETAILS" | grep -q "Discoverable: yes"; then
    ...
    ACTIVE_ADS=$(sed -n 's/.*ActiveInstances: 0x\([0-9A-Fa-f][0-9A-Fa-f]*\).*/\1/p' <<<"$BT_DETAILS" | sort -r | head -1)
...
if command -v hciconfig &>/dev/null; then
    HCI_OUTPUT=$(hciconfig -a 2>&1 || true)
    HCI_COUNT=$(grep -c '^hci[0-9]:' <<<"$HCI_OUTPUT" || true)
    HCI_UP=$(grep -c 'UP RUNNING' <<<"$HCI_OUTPUT" || true)
    if (( HCI_COUNT > 0 && HCI_UP == HCI_COUNT )); then
        pass_check "hciconfig: controller ทั้ง ${HCI_COUNT} ตัว UP RUNNING"
    else
        warn_check "hciconfig: พบ ${HCI_COUNT} controller, UP RUNNING ${HCI_UP} ตัว"
    fi
else
