...
<body class="flex flex-col md:flex-row min-h-screen">
    ...
    <aside id="sidebar" class="p-6 flex flex-col shadow-sm z-50" style="background: var(--bg-sidebar); border-right: 1px solid var(--border-color);">
        ...
        <div class="app-version sidebar-hide" aria-label="v2.12" title="v2.12">
            <span class="app-version-badge">v2.12</span>
        </div>
    </aside>
    ...
    <script>
        ...
        const _ROLE_CAPS = {
            super_admin: new Set([
                'patients:read', 'patients:write', 'devices:read', 'devices:write', 'pairing:write',
                'alerts:read', 'alerts:ack', 'alerts:settings:write',
                'users:manage', 'wards:manage', 'settings:global', 'audit:read', 'export:read'
            ]),
            ward_admin: new Set([
                'patients:read', 'patients:write', 'devices:read', 'devices:write', 'pairing:write',
                'alerts:read', 'alerts:ack', 'alerts:settings:write',
                'users:manage', 'audit:read', 'export:read'
            ]),
            staff_nurse: new Set(['patients:read', 'devices:read', 'alerts:read', 'alerts:ack', 'export:read']),
            viewer: new Set(['patients:read', 'devices:read', 'alerts:read'])
        };
        ...
        fetch('/api/me').then(async response => {
            ...
            // Apply role-based CSS class for legacy admin-only elements
            if (role === 'super_admin') document.body.classList.add('is-admin');
...
app.post('/api/login', async (req, res) => {
    ...
    // Audit login
    auditLog(user.id, 'login', 'user', user.id, { username }, null, req.ip).catch(() => { });
});
    ...
    res.send(ui('pats', `<div class="grid md:grid-cols-3 gap-8"><div class="admin-only card p-6 h-fit"><h3 class="font-bold mb-6">👥 เพิ่มคนไข้</h3><div class="space-y-4"><input id="p_hn" placeholder="HN" class="w-full border p-3 rounded-xl bg-slate-50"><input id="p_nm" placeholder="ชื่อ-สกุล" class="w-full border p-3 rounded-xl bg-slate-50"><button onclick="addP()" class="w-full bg-blue-600 text-white p-4 rounded-xl font-bold">บันทึก</button></div></div><div class="md:col-span-2 card overflow-hidden"><table><thead><tr><th>HN</th><th>Name</th><th class="admin-only"></th></tr></thead><tbody>${rows}</tbody></table></div></div>`, `
        window.addP = async () => {
            const response = await fetch('/api/patients', {
                method:'POST', headers:{'Content-Type':'application/json'},
                body:JSON.stringify({hn:document.getElementById('p_hn').value, nm:document.getElementById('p_nm').value})
            });
            if (!response.ok) return alert(await apiErrorMessage(response, 'ไม่สามารถเพิ่มผู้ป่วยได้'));
            location.reload();
        };
        window.editP = (hn, name) => {
            openModal('✏️ แก้ไข', '<input id="enm" value="'+escapeHTML(name)+'" class="w-full border p-3 rounded-xl bg-slate-50">', async () => {
                const response = await fetch('/api/patients/update', {
                    method:'POST', headers:{'Content-Type':'application/json'},
                    body:JSON.stringify({hn, newName:document.getElementById('enm').value})
                });
...
app.post('/api/patients', requireCapability('patients:write'), async (req, res) => {
    ...
    if (!hn || !name || hn.length > 50 || name.length > 200) {
    ...
    try {
        ...
        await pool.query('INSERT INTO patients (hn_number, name) VALUES ($1,$2)', [hn, name]);
        res.json({ success: true });
    } catch (error) {
...
app.post('/api/patients/update', requireCapability('patients:write'), async (req, res) => {
    ...
    try {
        ...
        const result = await client.query(
            'UPDATE patients SET name=$1 WHERE LOWER(hn_number)=LOWER($2) RETURNING id',
            [name, hn]
        );
        ...
        await client.query(
            'UPDATE nurseaid SET name=$1, lastupdate=NOW() WHERE LOWER(hm_number)=LOWER($2)',
            [name, hn]
        );
        await client.query('COMMIT');
        res.json({ success: true });
...
app.delete('/api/patients/:hn', requireCapability('patients:write'), async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(
            'DELETE FROM patients WHERE LOWER(hn_number)=LOWER($1) RETURNING id',
            [hn]
        );
...
app.get('/matching', requireCapability('pairing:write'), async (req, res) => {
    ...
    const cards = r.rows.map(x => {
        const btnHtml = x.hm_number
            ? `<button onclick="changeDevice('${escapeJsSingle(x.mac)}', '${escapeJsSingle(x.name)}', '${escapeJsSingle(x.hm_number)}', '${escapeJsSingle(x.bed_no || '')}')" class="admin-only w-full p-2 mb-2 rounded-lg text-[10px] font-bold transition-colors" style="background: var(--accent-amber); color: var(--text-inverse);">🔄 Change Device</button><button onclick="unpair('${escapeJsSingle(x.mac)}')" class="admin-only w-full p-2 rounded-lg text-[10px] font-bold" style="color: var(--accent-red); border: 1px solid var(--border-color);">Unpair</button>`
    ...
    res.send(ui('match', `<h2 class="text-xl font-bold mb-8">Pairing</h2><div id="pairing-grid" class="monitor-grid-layout">${cards}</div>`, `
...
app.get('/api/devices-available', requireCapability('pairing:write'), async (req, res) => {
    const r = await pool.query(
        'SELECT * FROM nurseaid WHERE hm_number IS NULL ORDER BY device_no ASC'
    );
    res.json(r.rows);
});
...
<body class="flex items-center justify-center min-h-[100dvh] bg-slate-900 font-['Prompt']">
    <main class="bg-white p-6 sm:p-10 rounded-3xl sm:rounded-[2.5rem] w-full max-w-sm shadow-2xl">
        ...
        <div class="mt-6 flex items-center justify-center" aria-label="v2.12" title="v2.12">
            <span class="rounded-full border border-slate-200 bg-slate-100 px-2 py-1 text-[10px] font-mono font-bold text-slate-500">v2.12</span>
        </div>
    </main>
    <script>
