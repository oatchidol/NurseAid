        ...
        if topic.startswith("ble/"):
            ...
            if topic == "ble/patient":
                ...
                for field, (measurement, influx_field) in vitals_map.items():
                    value = data_json.get(field)
                    # NOTE: Off-wrist filtering is handled by the BLE Gateway.
                    # The gateway only sends vitals that passed its quality gate,
                    # so we must NOT double-filter here or data will be lost.
