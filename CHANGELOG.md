...
### Added
- เพิ่มการดึงรายชื่อผู้ป่วยที่กำลัง Admit จาก HIS ตามรหัส Ward ในหน้า “จัดการผู้ป่วย” ผู้ใช้ต้องเลือก Ward ปลายทางที่ตนมีสิทธิ์ก่อน จากนั้นเลือกรายชื่อหนึ่งรายเพื่อเติม HN/ชื่อในฟอร์มเดิมและกดบันทึกเอง ระบบเรียก HIS ผ่าน authenticated server-side proxy, ไม่ cache/log payload, รักษาเลขศูนย์นำหน้าของ HN และไม่รับ field นอกเหนือจาก `hn`/`fullname`; URL และ timeout ตั้งผ่าน `OR_PATIENT_API_BASE_URL` / `OR_PATIENT_TIMEOUT_MS` โดยไม่ hard-code endpoint จริง
