...
### Added
- **อัปเดตเฟิร์มแวร์บอร์ดรับสัญญาณ ESP32 ผ่านหน้าเว็บ (OTA)** — อัปโหลดไฟล์ `.bin` ที่หน้า `/system-mgmt` แล้วสั่งติดตั้งลงบอร์ดได้เลย ไม่ต้องไล่ถอดบอร์ดมาเสียบ USB ทีละตัว
  - สั่งแบบ canary: ต้องผ่านบอร์ดทดลอง 1 ตัวก่อน ระบบจึงยอมให้เลือกหลายบอร์ดพร้อมกัน บังคับทั้งฝั่งเซิร์ฟเวอร์และหน้าจอ
  - แผงสถานะรายบอร์ดแบบสด แก้ไข/ลบเวอร์ชันที่อัปโหลดไว้ได้ — เดิมเป็น `prompt()`/`alert()` และผลการติดตั้งจริงถูกบันทึกลงฐานข้อมูลโดยไม่แสดงบนจอเลยแม้แต่บรรทัดเดียว
  - อ่านข้อมูลที่ฝังมาในไฟล์ `.bin` (NAidMETA) มาเทียบให้ดูก่อนกดติดตั้ง — เวอร์ชันเก่า/ใหม่, MQTT broker, จำนวนอุปกรณ์สูงสุด พร้อมเตือนเมื่อ broker ในเฟิร์มแวร์ใหม่ไม่ตรงกับที่บอร์ดใช้อยู่
  - คุมด้วย capability ใหม่ `devices:firmware:write` แยกจาก `devices:write`
- **ย้าย MQTT broker พร้อมกับ OTA ได้ โดยถอยกลับเองอัตโนมัติ** — ที่อยู่ใหม่ถูกเก็บเป็น "ของทดลอง" เท่านั้น จะยืนยันก็ต่อเมื่อเชื่อมต่อต่อเนื่องครบ 120 วินาที *และ* ได้รับรายชื่ออุปกรณ์จากแอปจริง (แค่ broker ยอมรับการเชื่อมต่อยังไม่พอเป็นหลักฐานว่าต่อถูกตัว) ถ้าไม่ผ่านใน 15 นาทีหรือรีบูตซ้ำ 2 ครั้ง บอร์ดถอยกลับไป broker เดิมเอง — พิมพ์ที่อยู่ผิดจึงไม่ทำให้ต้องเดินไปเสียบ USB
- **เฝ้าระวังบอร์ด ESP32 แบบถาวร พร้อมแจ้งเตือนเมื่อบอร์ดหาย** — เดิมรู้ว่าบอร์ดตายก็ต่อเมื่อมีคนเปิดหน้า `/esp32-mgmt` ดูเอง และ heartbeat ของบอร์ดที่ไม่มีชื่อในไฟล์ topology ถูกทิ้งทั้งหมด (ของจริง 7 บอร์ดส่งข้อมูล เข้าฐานข้อมูลถึงแค่ 1)
  - ทะเบียนบอร์ดถาวรในฐานข้อมูล ไม่ผูกกับไฟล์ snapshot อีกต่อไป, บันทึกประวัติออนไลน์/ออฟไลน์และการรีบูต, สรุป uptime 24 ชม. และ 7 วัน
  - แจ้งเตือนผ่าน alert engine (ค่าเริ่มต้น 5 นาที) โดย**เจตนาไม่เขียนลง `alert_logs`** เพราะตารางนั้นถูกอ่านแบบไม่แยกประเภท — ปัญหาทางเทคนิคต้องไม่ไปส่งเสียงเตือนใส่พยาบาล
  - ซ่อน / กู้คืน / ลบบอร์ดได้ บอร์ดที่ซ่อนไว้กลับมาเองทันทีที่พิสูจน์ว่ายังมีชีวิต และการลบถาวรถูกปฏิเสธถ้าบอร์ดยังส่งข้อมูลอยู่ใน 10 นาทีที่ผ่านมา
- **ตั้งค่า WiFi ให้บอร์ดจากหน้าเว็บได้** — เดิมเปลี่ยน WiFi ของหอผู้ป่วยทีต้องไล่แฟลชบอร์ดใหม่ทุกตัวผ่าน USB คำสั่งนี้เป็นการ *เพิ่ม* เครือข่ายเข้ารายการ ไม่ตัดการเชื่อมต่อที่ใช้อยู่ ณ ตอนนั้น รหัสผ่านไม่ถูกบันทึกลง audit log, response หรือ log ฝั่งเซิร์ฟเวอร์เลย
- **สถานะ "พักตามรอบ" ของนาฬิกา** — ผู้ป่วยระดับกลาง/ต่ำถูกตัดการเชื่อมต่อนาฬิกาเป็นช่วง ๆ เพื่อยืดแบต เดิมบอร์ดประกาศเรื่องนี้ด้วย payload ที่**เหมือนกันทุกไบต์**กับตอนนาฬิกาหลุดจากข้อมือ พยาบาลจึงแยกไม่ออกว่า "พักตามกำหนด" หรือ "นาฬิกาหลุดไปห้านาทีแล้ว" ตอนนี้เป็นสถานะของตัวเอง และ**ไม่ล้างค่าสัญญาณชีพ** เพราะค่าล่าสุดยังเป็นของจริงของผู้ป่วยจนกว่าจะถึงรอบวัดถัดไป
- **ผู้ช่วย AI เลือกข้อมูลเองด้วย tool calling** — เลิกใช้ตัวจำแนกคำถามแบบ regex ที่ต้องเดาให้ได้ก่อนที่โมเดลจะเห็นคำถามว่าเกี่ยวกับผู้ป่วยหรือไม่ เปลี่ยนเป็นให้โมเดลเรียกเครื่องมือเอง 3 ตัว (ค้นหาเตียง / ดูสัญญาณชีพ / ดูแนวโน้ม) และมีร่องรอยการเรียกเครื่องมือใต้คำตอบ เพื่อให้แยกออกว่าคำตอบมาจากการดึงข้อมูลจริงหรือการเดา
  - ข้อกำหนดความปลอดภัยที่ยังบังคับอยู่: ทุกเครื่องมืออ่าน snapshot สดใหม่และกรองตามสิทธิ์ผู้ถามซ้ำทุกครั้ง โมเดลระบุได้แค่ชื่อเตียง ไม่เคยได้รับ ward id, MAC หรือ HN — การเรียกเครื่องมือจึงขยายขอบเขตข้อมูลที่ผู้ถามเห็นอยู่แล้วไม่ได้
- ส่งระดับความสำคัญของผู้ป่วยไปยังบอร์ดผ่าน MQTT (`ble/priority`) เพื่อให้บอร์ดจัดตารางการวัดตามระดับที่พยาบาลตั้งไว้จริง
- หน้า "ตรวจสอบอัปเดต" แสดง release notes ของทุกเวอร์ชันที่ข้ามมา ไม่ใช่เฉพาะเวอร์ชันล่าสุด พร้อมแก้ลิงก์ไปหน้า tag ที่ 404 ทุกครั้งเพราะขาดตัว `v` นำหน้า
- กราฟแนวโน้มสัญญาณชีพมีอนิเมชันกวาดจากซ้ายไปขวาแบบจอมอนิเตอร์ทางการแพทย์, เส้นเรืองแสงตามธีม และจุดกะพริบที่ค่าล่าสุด — ข้ามอนิเมชันให้อัตโนมัติเมื่อผู้ใช้ตั้ง `prefers-reduced-motion`
- การ์ดผู้ป่วยในแถวเดียวกันสูงเท่ากัน และช่อง HR/SpO₂/อุณหภูมิ มีไอคอนประจำค่า เพื่อให้อ่านเป็นเครื่องมือคนละตัวได้ในการกวาดตาครั้งเดียว

### Changed
- **ผู้ป่วยที่ยังไม่ได้ตั้งระดับความสำคัญ = `กลาง` แล้ว** ทั้งระบบ (schema default, backfill เฉพาะแถวที่เป็น NULL, ค่าที่ส่งเข้า MQTT, ป้ายบนการ์ด และ dropdown) — เดิม "ไม่ระบุ" เป็นสถานะที่สาม ที่ UI, ตัวตั้งเวลา poll และ AI brief ต่างคนต่างเดาไปคนละทาง การ backfill แตะเฉพาะค่า NULL เท่านั้น ผู้ป่วยที่พยาบาลตั้ง `สูง` ไว้แล้วไม่ถูกแตะ
- **ลดเวลาพักระหว่างรอบวัดของระดับกลาง/ต่ำเหลือ 1 และ 5 นาที** (เดิม 5 และ 10 นาที ซึ่งนานเกินกว่าที่หอผู้ป่วยยอมรอค่าได้) ค่านี้ถูกคัดลอกด้วยมือไว้หลายที่ ทั้ง freshness floor, เกณฑ์แจ้งเตือนอุปกรณ์หลุด และช่วงเวลา query ของ InfluxDB จึงถูกไล่แก้ตามทั้งหมด — ถ้าแก้แต่เฟิร์มแวร์ ระดับกลางจะยังรอแจ้งเตือนอุปกรณ์หลุดถึง 8 นาที และการแจ้งเตือนสัญญาณชีพจะถูกกลบตลอดช่วงนั้น ซึ่งไม่ใช่แค่ช้า แต่เป็นรูโหว่ของการแจ้งเตือน ตอนนี้กลาง/ต่ำแจ้งเตือนที่ 4 และ 8 นาที
- ระดับ `กลาง` เปลี่ยนเป็นสีน้ำเงิน (เดิมเป็นสีเทาสเลตซึ่งมองไกล ๆ แยกจาก `ต่ำ` ไม่ออก ทั้งที่เป็นสองระดับที่เปลี่ยนความถี่การวัดจริง) และ optgroup ในตัวเลือกบอกค่าใช้จ่ายด้านแบตของแต่ละระดับ — เลือกไว้เป็น optgroup เพราะป้ายจะโผล่เฉพาะตอนเปิดรายการ ตัวชิปที่ปิดอยู่จึงไม่ยืดจนการ์ดผู้ป่วยเสียรูป
- การ์ดตั้งค่าแจ้งเตือนรายผู้ป่วยแสดง**เกณฑ์ที่มีผลจริง** และบอกว่าระดับความสำคัญใดเป็นตัวขยาย — เดิมพิมพ์ค่าที่ตั้งไว้ (2 นาที) ขณะที่ alert engine ใช้ค่าที่ถูกขยายแล้ว (8 นาที)
- หัวหน้า `/esp32-mgmt` ยุบจาก 4 ไทล์กับแถบแดงแยก เหลือแถบสรุปเดียวที่ตอบเป็นคำพูดว่าตอนนี้มีบอร์ดต้องไปดูหรือไม่ และบอกชื่อบอร์ดนั้น ทุกตัวเลขมาจากรายการบอร์ดชุดเดียวกับที่การ์ดข้างล่างใช้ หัวหน้าจึงขัดกับการ์ดไม่ได้
- `npm test` ใช้งานได้จริงแล้ว — เดิมอ้างไฟล์ที่ถูก gitignore ไว้และไม่มีอยู่ใน clone ใด ๆ จึงล้มก่อนรันอะไรเลย ขณะที่ไฟล์ใน `test/` ไม่เคยถูกเรียกสักไฟล์ ตอนนี้แยกเป็น `test:offline` (119 tests, ไม่ต้องใช้ DB/MQTT) และ `test:integration` ที่ขึ้นบรรทัด SKIP ชัดเจนเมื่อไม่มี stack

### Fixed
- **โค้ดฝั่งเบราว์เซอร์ที่ backslash หายไปใน template literal 2 จุด** — `server.js` ส่งโค้ดเบราว์เซอร์ออกไปในรูป template literal ซึ่งกิน backslash เดี่ยวทิ้ง: ช่องกรอกที่อยู่ broker ใช้ `\d` ที่กลายเป็นตัวอักษร `d` จึงปฏิเสธ IPv4 ที่ถูกต้องทุกค่า และตัวตรวจช่องว่างในฟอร์ม WiFi ใช้ `\s` ที่กลายเป็น `s` จึงปัดตก SSID ที่มีตัว s แต่ปล่อยค่าที่มีช่องว่างผ่านไปได้ (ฝั่งเซิร์ฟเวอร์ยังกันไว้ตลอด ไม่มีค่าเสียหลุดถึงบอร์ด)
- นาฬิกาที่หลุดไปแล้วยังถูกวาดเป็น "เชื่อมอยู่" สีเขียวบนการ์ดที่แสดงบอร์ดว่าออฟไลน์อยู่พร้อมกัน — ตัว parser อ่านแต่ id ของนาฬิกาแล้วทิ้งสถานะที่ collector ใส่มาให้
- นาฬิกาหนึ่งเรือนโผล่ใต้บอร์ดหลายตัวพร้อมกัน เพราะทะเบียนแทนที่รายการของบอร์ดเฉพาะตอนบอร์ดนั้นรายงานเข้ามาเอง ตอนนี้บอร์ดที่เพิ่งรายงานเป็นเจ้าของนาฬิกาที่รายงาน และ cache ที่ขัดกันเองตัดสินด้วยอายุข้อมูลที่สดที่สุด (ค่าติดลบนับเป็นข้อมูลเสีย ไม่ใช่สดที่สุด)
- กด "ซ่อน" บอร์ดแล้วไม่มีอะไรเกิดขึ้น — query ไม่ได้ SELECT คอลัมน์ `revoked_at` ที่ตัวกรองใช้ บอร์ดเดียวกันจึงโผล่ทั้งในรายการปกติและในรายการที่ซ่อนไว้พร้อมกัน
- ลบบอร์ดแล้วบอร์ดกลับมาใหม่ในรอบ poll ถัดไปพร้อมข้อมูลว่างเปล่า และ API ทั้งซ่อนและลบตอบ 404 หลังจากนั้น (บอร์ด 2 ตัวค้างอยู่บนหน้าโดยไม่มี API ใดลบได้) — เพิ่ม tombstone บันทึกว่าบอร์ดถูกปลดระวางแล้ว
- เตียงแสดงสัญญาณชีพของผู้ป่วยที่ไม่มีใครเฝ้าอยู่ 2 ทาง: หน้าต่างความสดของ "สถานะ" หมดอายุเร็วกว่าหน้าต่างของค่าที่สถานะนั้นรับรอง ระบบจึงเดา "ใส่อยู่" จากค่าที่กำลังสงสัยความน่าเชื่อถือเสียเอง และค่าที่ประทับเวลาไว้ในอนาคตถูกนับเป็นข้อมูลสดที่สุด (เครื่องนี้ไม่มีแบตเตอรี่ RTC และ udp/123 ถูกบล็อก การถอยเวลากลับจึงเกิดจริงเป็นประจำ)
- เส้นทางดาวน์โหลดเฟิร์มแวร์: ถูก redirect ไป `/login` โดย auth middleware, ส่งแบบ chunked จนบอร์ดฟ้อง "Server Did Not Report Size", สร้าง URL เป็น `https://` ที่บอร์ดเปิดไม่ได้ (ไม่มี TLS), ไฟล์ที่อัปโหลดหายทุกครั้งที่สร้าง container ใหม่ และ volume ใหม่ถูกสร้างเป็นของ root ทั้งที่แอปรันเป็น `appuser`
- `wifi-reset` ไม่ได้รีเซ็ตจริง — มันล้างของ WiFiManager แต่ตอนบูตอ่านรายการที่บันทึกไว้ก่อน บอร์ดจึงต่อกลับเครือข่ายเดิมเหมือนไม่มีอะไรเกิดขึ้น พร้อมย้ายรหัสผ่าน WiFi ออกจาก git และหยุด echo ทุก payload ของคำสั่งไปยัง log ที่วิ่งบน MQTT port 1883 แบบไม่เข้ารหัส
- ค่า MQTT broker ที่คอมไพล์มากับเฟิร์มแวร์ชี้ไปที่ IP ที่ route ไม่ถึง ซึ่งกลายเป็นอันตรายเมื่อมีระบบย้าย broker: บอร์ดที่ไม่เคยย้ายจะ fallback มาที่ค่านี้และขึ้นมาโดยไม่มี broker ที่ตอบเลย
...
- หน้า "จัดการผู้ป่วย" ปรับลำดับให้ค้นหาจาก HIS ก่อนเสมอ ฟอร์มกรอกเอง (HN/ชื่อ-สกุล) ซ่อนไว้เป็นค่าเริ่มต้นและจะโผล่ให้กรอกก็ต่อเมื่อเลือกผลลัพธ์จาก HIS แล้ว หรือค้นหาแล้วไม่พบ/HIS ใช้งานไม่ได้ — เดิมทั้งสองช่องทางแสดงพร้อมกันตลอดเวลาไม่มีลำดับความสำคัญ
...
- แยกปุ่ม "เริ่มต้นใช้งาน" (Quick Setup Wizard) ออกมาเป็นเมนูของตัวเองบน sidebar แยกจากเมนูกิจกรรมหลัก (มอนิเตอร์/รายงาน/อุปกรณ์/ผู้ป่วย/จับคู่อุปกรณ์/หอผู้ป่วย/ผู้ใช้งาน) — เดิมปนอยู่ในกลุ่มเดียวกัน ทำให้ wizard ดูเหมือนเป็นกิจกรรมหลักอันหนึ่งทั้งที่จริงเป็นทางลัดแนะนำขั้นตอนสำหรับตั้งค่าเริ่มต้น
- ขั้นตอน "ผู้ป่วย" ใน Quick Setup Wizard ค้นหาผู้ป่วยจาก HIS ได้แล้วเหมือนหน้าจัดการผู้ป่วย — เลือก Ward แล้วค้นหาด้วย HN/ชื่อ-สกุล/เลขบัตรประชาชน เลือกผลลัพธ์เพื่อเติมฟอร์มเดิมให้ตรวจสอบก่อนกดเพิ่ม หรือกรอกเองได้ทันทีถ้าค้นหาไม่พบ/HIS ใช้งานไม่ได้ ใช้ endpoint `/api/or-patients` เดิม ไม่มี endpoint ใหม่
